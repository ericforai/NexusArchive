...
public class YonSuiteClient {
    ...
    /**
     * 付款申请单列表查询
     * POST /yonbip/EFI/paymentApply/list
     * 文档: 付款申请单列表查询
     *
     * @param accessToken 可选, 为null则自动获取
     * @param request 付款申请单列表查询请求
     * @return 付款申请单列表
     */
    public YonPaymentApplyListResponse queryPaymentApplyList(String accessToken, YonPaymentApplyListRequest request) {
        String token = getToken(accessToken);
        String url = baseUrl + "/yonbip/EFI/paymentApply/list"
                + "?access_token=" + URLEncoder.encode(token, StandardCharsets.UTF_8);

        log.info("Calling YonSuite queryPaymentApplyList: pageIndex={}, pageSize={}",
        ...
        try {
            ...
            log.info("YonSuite payment apply list response: {}", respStr);

            if (respStr == null || respStr.isEmpty()) {
                YonPaymentApplyListResponse emptyResponse = new YonPaymentApplyListResponse();
                emptyResponse.setCode("200");
                emptyResponse.setMessage("No data");
                emptyResponse.setData(new YonPaymentApplyListResponse.PaymentApplyData());
                return emptyResponse;
            }

            YonPaymentApplyListResponse response = objectMapper.readValue(respStr, YonPaymentApplyListResponse.class);

            if (!"200".equals(response.getCode())) {
                log.warn("YonSuite payment apply list API warning: {} - {}", response.getCode(), response.getMessage());
            }

            return response;
        } catch (Exception e) {
            log.error("Failed to call YonSuite queryPaymentApplyList", e);
            throw new RuntimeException("YonSuite API Error", e);
        }
    }
