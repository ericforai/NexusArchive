...
public class YonSuitePaymentApplyClient {
    ...
    public List<VoucherDTO> syncPaymentApplyFiles(ErpConfig config, LocalDate startDate, LocalDate endDate) {
        ...
        if (accessToken == null) {
            log.error("无法获取 access token，跳过文件同步");
        ...
        log.info("查询到 {} 个付款申请单 ID，开始获取附件 ID", paymentApplyIds.size());

        // 2. 查询每个付款申请单的附件 ID
        List<String> attachmentIds = queryAttachmentIds(config, accessToken, paymentApplyIds);

        if (attachmentIds.isEmpty()) {
            log.info("未查询到任何附件 ID，跳过文件同步");
            return Collections.emptyList();
        }

        log.info("查询到 {} 个附件 ID，开始同步文件", attachmentIds.size());

        // 3. 分批同步文件
        return syncPaymentApplyFilesBatched(config, attachmentIds);
    }
     ...
     * @param paymentApplyIds 付款申请单 ID 列表
     * @return 附件 ID 列表
     */
    private List<String> queryAttachmentIds(ErpConfig config, String accessToken, List<String> paymentApplyIds) {
        List<String> allAttachmentIds = new ArrayList<>();

        for (String paymentApplyId : paymentApplyIds) {
            try {
                // 调用通用附件查询接口
                var attachmentResponse = yonSuiteClient.queryAttachments(accessToken, paymentApplyId);

                if (attachmentResponse != null && "200".equals(attachmentResponse.getCode())
                        && attachmentResponse.getData() != null) {
                    for (var attachment : attachmentResponse.getData()) {
                        if (attachment.getId() != null) {
                            allAttachmentIds.add(attachment.getId());
                            log.debug("找到附件: paymentApplyId={}, attachmentId={}, fileName={}",
                                    paymentApplyId, attachment.getId(), attachment.getFileName());
                        }
                    }
                }
            } catch (Exception e) {
                log.warn("查询附件失败: paymentApplyId={}, error={}", paymentApplyId, e.getMessage());
            }
        }

        log.info("从 {} 个付款申请单中找到 {} 个附件", paymentApplyIds.size(), allAttachmentIds.size());
    ...
    private List<VoucherDTO> convertToVoucherDTOs(List<YonPaymentApplyFileResponse.FileData> fileList) {
        ...
        for (YonPaymentApplyFileResponse.FileData fileInfo : fileList) {
            // 生成唯一的凭证号：PAYMENT_APPLY_ + 文件ID
            String voucherNo = "PAYMENT_APPLY_" + fileInfo.getId();

            log.info("转换付款申请单文件: fileId={}, fileName={}, voucherNo={}",
                    fileInfo.getId(), fileInfo.getFileName(), voucherNo);

            VoucherDTO dto = VoucherDTO.builder()
                    .voucherId(fileInfo.getId())
                    .voucherNo(voucherNo)  // 使用唯一的凭证号
