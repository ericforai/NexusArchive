...
public class ComplianceController {
    ...
    public Result<BatchComplianceResult> checkBatchArchiveCompliance(
            @Parameter(description = "档案ID列表", required = true)
    ...
    public Result<String> getComplianceReport(
        ...
        try {
            ...
            // 执行符合性检查 (In real world, we might want to fetch the PERSISTED report, but generating fresh is OK for on-demand)
            // Ideally check if a signed report exists in AuditLog first, but for now generating fresh is safer to reflect latest rule
            ComplianceCheckService.ComplianceResult result = complianceCheckService.checkCompliance(archive, files);
    ...
    public Result<ComplianceStatistics> getComplianceStatistics() {
        try {
            ...
            // 统计总数 (From audit logs or archive table? Better from AuditInspectionLog where is_compliant is set)
            // NOTE: Only counting latest inspection per archive is ideal, but for performance we might just count all latest logs
            // Here we do a simple aggregation on AuditInspectionLog where inspection_stage='patrol'
            ...
            // Non-compliant
             ...
             // Warnings (Assuming non-compliant means strict violation, but warnings are tricky if isCompliant=true but has warnings)
             // The current DB schema has 'compliance_warnings' text field.
             // Let's refine:
             // 1. Fully Compliant: is_compliant=true AND (compliance_warnings IS NULL OR empty)
             // 2. Compliant with Warnings: is_compliant=true AND compliance_warnings IS NOT NULL
            ...
            double rate = (totalChecked > 0) ? (strictCompliant.doubleValue() + compliantWithWarn.doubleValue()) / totalChecked * 100 : 0;
    ...
    private String generateXmlReport(Archive archive, ComplianceCheckService.ComplianceResult result) {
        ...
        // StandardReportGenerator works with FourNatureReport, but we have ComplianceResult here.
        // We will construct a simple XML specific for this "ComplianceResult" or we should map it.
        // For now, keep the simple XML construction as placeholder or enhance common generator later.
        // The Requirement said "StandardReportGenerator" which generates FourNatureReport.
        // Let's stick to the manual construction for this specific endpoint unless we map ComplianceResult -> FourNatureReport
    ...
    public static class BatchComplianceResult {
        ...
        public List<SingleResult> getResults() { return results; }
        ...
        public static class SingleResult {
            ...
            public ComplianceCheckService.ComplianceResult getResult() { return result; }
            public void setResult(ComplianceCheckService.ComplianceResult result) { this.result = result; }
        }
    }
    ...
    public static class ComplianceStatistics {
        ...
        public int getFullyCompliant() { return fullyCompliant; }
        public void setFullyCompliant(int fullyCompliant) { this.fullyCompliant = fullyCompliant; }
        public int getCompliantWithWarnings() { return compliantWithWarnings; }
        public void setCompliantWithWarnings(int compliantWithWarnings) { this.compliantWithWarnings = compliantWithWarnings; }
        public int getNonCompliant() { return nonCompliant; }
        public void setNonCompliant(int nonCompliant) { this.nonCompliant = nonCompliant; }
