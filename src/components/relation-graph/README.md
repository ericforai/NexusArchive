...
| 文件 | 角色 | 功能 |
| --- | --- | --- |
| `ThreeColumnLayout.tsx` | 三栏布局组件 | **默认视图**：左侧上游、中心核心、右侧下游（支持逐层展开） |
...
| **关系分类** | 自动根据关系方向分层展示上游/下游（支持多跳关系） |
| **逐层展开** | 默认展示首层关系，可逐层展开到更深业务链路 |
| **主线高亮** | 支持付款主线节点高亮与缺口告警展示 |
| **方向优先** | 优先使用后端 `directionalView` 进行上下游分栏，前端仅作兜底推断 |
