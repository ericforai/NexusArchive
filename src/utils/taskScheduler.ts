...
export const loadTasksFromStorage = (): ScheduledTask[] => {
  try {
    const stored = safeStorage.getItem(STORAGE_KEYS.TASKS);
    if (stored) {
...
export const loadHistoryFromStorage = (): TaskExecutionHistory[] => {
  try {
    const stored = safeStorage.getItem(STORAGE_KEYS.HISTORY);
    if (stored) {
      return JSON.parse(stored);
    }
  } catch (e) {
...
export const saveHistoryToStorage = (history: TaskExecutionHistory[]) => {
  try {
    ...
    safeStorage.setItem(STORAGE_KEYS.HISTORY, JSON.stringify(limited));
  } catch (e) {
