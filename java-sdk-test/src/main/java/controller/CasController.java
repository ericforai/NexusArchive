...
public class CasController {
	...
	public String onAuthCodeBack1(HttpServletRequest request) {
		try {
			HashMap<String, String> map = new HashMap<String, String>();
			map.put("ticket", "ticket");
			String res = HttpUtil.doFormPost("https://192.168.1.1/iam/auth/cas/serviceValidate", map);
			if (StringUtils.isEmpty(res)) {
				return "接口调用失败";
			}
			String userJson = parseCasXml(res);
	...
	private static String parseCasXml(String res){
		try {
			DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
			DocumentBuilder builder = factory.newDocumentBuilder();
			Document doc = builder.parse(new ByteArrayInputStream(res.getBytes()));
			Element authenticationSuccess = (Element) doc.getElementsByTagName("cas:authenticationSuccess").item(0);
			Element userElement = (Element) authenticationSuccess.getElementsByTagName("cas:user").item(0);
			String urlEncodedJSON = userElement.getTextContent();
			String decodedJSON = java.net.URLDecoder.decode(urlEncodedJSON, "UTF-8");
			return decodedJSON;
		}catch (Exception e){
			e.printStackTrace();
