...
public class OidcController {
	...
	public String loginWithSso() {
		...
		//authUrl 为零信任认证地址,具体以零信任部署的ip地址为准
		String authUrl="https://192.168.1.1";
		sBuilder.append(authUrl).append("?service=").append(URLEncoder.encode("redirectUrl"));
	...
	/**
	 * 根据零信任重定向回来的code换取令牌accessToken,根据accessToken 获取用户信息
	 * client_secret 零信任申请的应用密钥
	 * client_id 零信任申请的应用ID
	 * 接口请求地址为零信任的认证页地址
	 */
	@RequestMapping(value = "/authBack")
	public void onAuthCodeBack(HttpServletRequest request) {
		...
		try {
			...
			tokenNameValuePairs.add(nvp);
			String tokenJson = HttpUtils.getJsonFromServer("https://192.168.1.103/iam/auth/oidc/accessToken", tokenNameValuePairs);
			...
			String json = HttpUtils.getJsonFromServer("https://192.168.1.103/iam/auth/oidc/userInfo", nameValuePairs);
