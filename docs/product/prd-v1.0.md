...
*   **功能逻辑**:
    1.  **创建盒子**: 输入“所属全宗”、“会计年度”、“档案类型”、“保管期限” -> 系统自动生成 `box_code` (规则：`全宗-年份-类型-流水号`)。
    2.  **装盒操作**: 扫描/勾选档案条目 -> 绑定到该 `box_code`。
    3.  **打印标签**: 生成盒脊标签 PDF（含二维码、盒号、起止日期、全宗号）。
    4.  **标签格式**: `fonds_no-archive_year-retention-serial`（示例：`JD-2025-永久-001`）。
*   **数据字段**: `arc_box` (`id`, `box_code`, `fonds_no`, `location_id`, `status`), `arc_box_item`（盒-档案关联）。
...
#### 1.5 实物位置管理与生命周期 (P1)
*   **用户故事**: 作为库房管理员，我需要对档案盒的库房/密集架/层/格进行精确定位，并跟踪实物状态流转。
*   **功能逻辑**:
    1.  建立库房位置模型：库房 -> 排/列 -> 架 -> 层 -> 格（支持编码规则与二维码）。
    2.  档案盒绑定位置，移动位置需审批并记录操作人/时间。
    3.  实物状态流转：`IN_STOCK` -> `BORROWED` -> `PENDING_RETURN` -> `RETURNED` -> `TO_DESTROY` -> `DESTROYED`。
*   **验收标准**: 任一盒可定位到具体格位；状态流转完整留痕可追溯。
